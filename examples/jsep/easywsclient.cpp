...
easywsclient::WebSocket::pointer from_url(const std::string& url, bool useMask, const std::string& origin) {
    ...
    }
    {
        ...
        int status;
        int i;
        line += sprintf(line, "GET /%s HTTP/1.1\r\n", path);
        if (port == 80) {
            line += sprintf(line, "Host: %s\r\n", host);
        }
        else {
            line += sprintf(line, "Host: %s:%d\r\n", host, port);
        }
        line += sprintf(line, "Upgrade: websocket\r\n");
        line += sprintf(line, "Connection: Upgrade\r\n");
        if (!origin.empty()) {
            line += sprintf(line, "Origin: %s\r\n", origin.c_str());
        }
        line += sprintf(line, "Sec-WebSocket-Key: x3JJHMbDL1EzLkh9GBhXDw==\r\n");
        line += sprintf(line, "Sec-WebSocket-Version: 13\r\n");
        line += sprintf(line, "\r\n"); ::send(sockfd, buf, strlen(buf), 0);
        memset(buf, 0, sizeof(buf)); line = buf;
